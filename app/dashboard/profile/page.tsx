...
export default function ProfilePage() {
  ...
  const enrolledActivities = userEnrollments.map(e => mockActivities[e.activityId]).filter(Boolean);

  const ageGroupLabel = user?.ageGroup === 'teen' ? '13-17 years' : '18-24 years';
          ...
          <div className="md:col-span-1">
            <Card className="p-6 space-y-6">
              ...
              <div className="flex flex-col items-center text-center">
                ...
                <p className="text-sm text-muted-foreground mt-1">{user?.ageGroup === 'teen' ? 'Teen' : 'Young Adult'}</p>
              </div>
              ...
              <div className="space-y-3 pt-6 border-t border-border">
                ...
                <div className="flex justify-between items-center">
                  <span className="text-sm text-muted-foreground">Activities Joined</span>
                  <span className="text-xl font-bold text-foreground">{userEnrollments.length}</span>
                </div>
                <div className="flex justify-between items-center">
          ...
          <div className="md:col-span-2 space-y-6">
            ...
            <Card className="p-6 space-y-4">
              ...
              {user?.interests && user.interests.length > 0 ? (
                ...
                <p className="text-sm text-muted-foreground">No interests selected yet. Update your profile to add interests.</p>
              )}
            </Card>
            ...
            <Card className="p-6 space-y-4">
              <div className="flex items-center justify-between">
                ...
                <span className="text-sm text-muted-foreground">{enrolledActivities.length}</span>
              </div>

              {enrolledActivities.length > 0 ? (
                <div className="space-y-2">
                  {enrolledActivities.slice(0, 5).map(activity => (
                    <div key={activity.id} className="p-3 rounded-lg bg-muted/50 hover:bg-muted transition">
                ...
                <p className="text-sm text-muted-foreground">You haven&apos;t joined any activities yet. Browse the activity feed to get started!</p>
              )}
            </Card>
          </div>
        </div>
